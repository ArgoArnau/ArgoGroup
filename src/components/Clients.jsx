      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6">
          {clients.map((client) => (
            <div
              key={client.name}
              className="group flex flex-col items-center justify-center p-6 bg-dark-surface border border-dark-border rounded-xl hover:border-gold/40 transition-all duration-300 hover:-translate-y-0.5 aspect-square"
            >
              ...
              <span className="text-gray-500 text-xs text-center leading-tight group-hover:text-gray-300 transition-colors">{client.name}</span>
            </div>
          ))}
        </div>
      </div>
